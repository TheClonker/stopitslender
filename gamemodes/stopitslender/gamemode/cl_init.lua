...
function GM:PreDrawOpaqueRenderables()//HUDPaint()PostDrawOpaqueRenderables	
	...
	if MySelf:Team() ~= TEAM_SLENDER then
		...
		if dead then
			draw.SimpleText("BATTERY DEAD", "Tahoma_lines18",batX, batY+batH+5, Color(215,15,15,100), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
		end
		...
		draw.SimpleText(txt, "Tahoma_lines50",w-gap*1.3+((MySelf:Alive() and 0) or 4), gap*1.2, Color(red,green,15,100*math.Round(math.Clamp(math.sin(RealTime()*3.5)*2,0,1))), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
		draw.SimpleText(time, "Tahoma_lines18",w-gap*1.3, gap*1.2+45, Color(255,255,255,100), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
	...
	if IsValid(MySelf:GetObserverTarget()) then
		...
		draw.SimpleText("Spectating "..name, "Tahoma_lines18",w-gap*1.3, h-gap*1.3-23, Color(255,255,255,100), TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP)
		
		local pages = MySelf:GetObserverTarget():IsPlayer() and MySelf:GetObserverTarget():GetPages() or 0
		draw.SimpleText("Pages "..pages.."/"..MySelf:GetMaxPages(), "Tahoma_lines18",w-gap*1.3, h-gap*1.3, Color(255,255,255,100), TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP)
		...
		if MySelf:Team() ~= TEAM_SLENDER then
			draw.SimpleText("Pages "..MySelf:GetPages().."/"..MySelf:GetMaxPages(), "Tahoma_lines18",w-gap*1.3, h-gap*1.3, Color(255,255,255,100), TEXT_ALIGN_RIGHT,TEXT_ALIGN_TOP)
		end
	end
	...
	if scoreboard_alpha > 0 then
		...
		draw.SimpleText(math.random(40) == 1 and "NO DATA" or "Stop it, Slender! by NECROSSIN", "Tahoma_lines23",sX+sW-6,sY+4-23*2-8, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
		draw.SimpleText(math.random(40) == 1 and "NO DATA" or "version "..(self.Version or "error"), "Tahoma_lines23",sX+sW-6,sY+4-23-6, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
		...
		if IsValid(slender) then
			...
			draw.SimpleText(name, "Tahoma_lines23",sX+6,sY+4, Color(215,15,15,scoreboard_alpha), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
			draw.SimpleText(ping, "Tahoma_lines23",sX+sW-6,sY+4, Color(215,15,15,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
			draw.SimpleText(muted and "MUTED" or "", "Tahoma_lines23",sX+sW-6-45,sY+4+offsetY, Color(215,15,15,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
		...
		for _,pl in ipairs(player.GetAll()) do
			...
			draw.SimpleText(!pl:Alive() and math.random(30) == 1 and "NO DATA" or name, "Tahoma_lines23",sX+6,sY+4+offsetY, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
			draw.SimpleText(ping, "Tahoma_lines23",sX+sW-6,sY+4+offsetY, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
			draw.SimpleText(pl:IsMuted() and "MUTED" or "", "Tahoma_lines23",sX+sW-6-45,sY+4+offsetY, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
		...
		draw.SimpleText("Click on squeaky player to mute!", "Tahoma_lines23",sX+4,sY+sH-4, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_LEFT,TEXT_ALIGN_TOP)
	...
	if VotingTime and VotingTime > CurTime() then
		...
		draw.SimpleText("Vote for the next map!", "Tahoma_lines23",mX+6,mY+4, Color(215,15,15,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
		...
		for ind = 1 + 8*(vote_cur_page - 1),math.min(#GAMEMODE.Maps,8*vote_cur_page) do
			...
			if tbl then
				...
				draw.SimpleText(num..".", "Tahoma_lines23",mX+6,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
				draw.SimpleText((tbl.map or "error!").."  ("..(tbl.votes or 0)..")", "Tahoma_lines23",mX+6+30,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
		...
		if voting_get_max_pages() > 1 then
			...
			if vote_cur_page > 1 then
				draw.SimpleText("9.", "Tahoma_lines23",mX+6,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
				draw.SimpleText("Previous page", "Tahoma_lines23",mX+6+30,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
				offsetY = offsetY + 23 + 2
			end
			
			if vote_cur_page < voting_get_max_pages() then
				draw.SimpleText("0.", "Tahoma_lines23",mX+6,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
				draw.SimpleText("Next page", "Tahoma_lines23",mX+6+30,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
			end
